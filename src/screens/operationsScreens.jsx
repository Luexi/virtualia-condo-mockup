...
import * as D4 from "../data/mockData.js";
import { PageHeader as PH4, Card as C4, Badge as B4, Ic as I4, QR as QR4, Avatar as AV4 } from "../components/ui.jsx";

const { createElement: s4 } = React;
...
function Comunicados({ showToast }) {
  ...
  const modal = draft && s4(React.Fragment, null,
    ...
    s4('div', { className: 'modal modal--lg' },
      s4('div', { className: 'modal__header' }, s4('div', { className: 'row row--between' },
        ...
        s4('button', { className: 'icon-btn', 'aria-label': 'Cerrar comunicado', onClick: () => setDraft(false) }, s4(I4, { name: 'X', size: 18 })),
      )),
        ...
        s4('div', { className: 'field' },
          s4('div', { className: 'row row--between' },
            ...
            s4('button', { className: 'btn btn--soft btn--sm', onClick: () => setText('Estimados residentes: les informamos que el día martes 3 de junio se realizará un corte de agua programado de 09:00 a 14:00 por mantenimiento a la cisterna. Agradecemos tomar previsiones. — Administración.') },
              s4(I4, { name: 'Sparkles', size: 14 }), 'Preparar borrador'),
          ),
          ...
          s4('div', { className: 'field__hint row', style: { gap: 6 } }, s4(I4, { name: 'Info', size: 13 }), 'Prepara un borrador para revisión antes de enviarlo a la comunidad.'),
        ),
      ),
      s4('div', { className: 'modal__footer' },
        ...
        s4('button', { className: 'btn btn--primary', onClick: () => { setDraft(false); showToast(`Comunicado enviado a: ${seg}`); } }, s4(I4, { name: 'Send', size: 15 }), 'Enviar'),
      ),
    ),
  );
  ...
  return s4('main', { className: 'page fade-in' },
    s4(PH4, { route: 'comunicados', actions: [
      s4('button', { key: 1, className: 'btn btn--secondary', onClick: () => showToast('Documento preparado') }, s4(I4, { name: 'Plus', size: 16 }), 'Subir documento'),
        ...
        s4(C4, { title: 'Avisos recientes', icon: 'Megaphone', body: false,
          ...
          s4('div', null, D4.COMUNICADOS.map((c, i) => s4('div', { key: i, style: { padding: '16px 20px', borderBottom: i < 3 ? '1px solid var(--border)' : 'none' } },
            ...
            s4('div', { className: 'row', style: { gap: 12 } },
              ...
              s4('button', { className: 'btn btn--ghost btn--sm', onClick: () => showToast('Detalle de lectura abierto') }, 'Detalle'),
            ),
          ))),
        ),
        s4('div', { className: 'banner banner--teal' }, s4('span', { className: 'banner__icon' }, s4(I4, { name: 'Sparkles', size: 17 })),
          s4('div', null, s4('div', { className: 'banner__title' }, 'Borrador asistido'),
            'Redacta avisos más rápido. Prepara el texto base para revisión antes del envío. ',
            s4('button', { className: 'card__link', style: { display: 'inline-flex' }, onClick: () => setDraft(true) }, 'Preparar borrador'))),
      ),
      s4(C4, { title: 'Documentos de la comunidad', icon: 'FolderOpen', body: false,
        headerRight: s4('button', { className: 'card__link', onClick: () => showToast('Documento preparado') }, s4(I4, { name: 'Plus', size: 14 }), 'Subir') },
        s4('div', { style: { padding: '6px 8px' } }, D4.DOCUMENTOS.map((d, i) => s4('div', { key: i, className: 'row row--between', style: { padding: '11px 12px', borderBottom: i < 4 ? '1px solid var(--border)' : 'none' } },
          ...
          s4('button', { className: 'icon-btn', 'aria-label': `Descargar ${d.nombre}`, onClick: () => showToast('Documento descargado') }, s4(I4, { name: 'Download', size: 16 })),
        ))),
      ),
    ),
    modal,
...
function Accesos({ showToast }) {
  ...
  return s4('main', { className: 'page fade-in' },
    s4(PH4, { route: 'accesos', actions: [
      s4('span', { key: 0, className: 'badge badge--green', style: { alignSelf: 'center' } }, s4('span', { className: 'badge__dot' }), 'Caseta activa'),
      s4('button', { key: 1, className: 'btn btn--primary', onClick: () => showToast('Código de visita generado') }, s4(I4, { name: 'Plus', size: 16 }), 'Generar acceso'),
    ] }),
    s4('div', { className: 'grid-main' },
      s4('div', { className: 'stack' },
        s4(C4, { title: 'Autorizaciones', icon: 'KeyRound', body: false,
          ...
          s4('div', { className: 'table-wrap' }, s4('table', { className: 'tbl' },
            s4('thead', null, s4('tr', null, ['Código', 'Visitante', 'Unidad', 'Tipo', 'Vehículo', 'Vence', 'Estado'].map((t, i) => s4('th', { key: i }, t)))),
      ...
      s4('div', { className: 'stack' },
        s4(C4, { title: 'Pase de acceso', icon: 'ScanLine', iconTone: 'sky' },
          ...
          s4('div', { className: 'status-tag', style: { marginTop: 14 } }, 'Código de acceso'),
        ),
        s4(C4, { title: 'Vista de vigilancia', icon: 'Shield', iconTone: 'slate' },
          s4('div', { className: 'banner banner--sky', style: { fontSize: 12.5 } }, s4('span', { className: 'banner__icon' }, s4(I4, { name: 'Info', size: 15 })), 'Modo simplificado para caseta: escanear código, registrar entrada/salida y consultar autorizaciones del día.'),
          s4('div', { className: 'row', style: { gap: 8, marginTop: 12 } },
            s4('button', { className: 'btn btn--primary btn--block', onClick: () => showToast('Código escaneado') }, s4(I4, { name: 'ScanLine', size: 16 }), 'Escanear'),
            s4('button', { className: 'btn btn--secondary btn--block', onClick: () => showToast('Registro manual abierto') }, 'Manual'),
          ),
        ),
      ),
    ),
...
function Reportes({ showToast }) {
  ...
  return s4('main', { className: 'page fade-in' },
    s4(PH4, { route: 'reportes', actions: [
      s4('button', { key: 1, className: 'btn btn--ghost', onClick: () => showToast('Excel generado') }, s4(I4, { name: 'Sheet', size: 16 }), 'Excel'),
      s4('button', { key: 2, className: 'btn btn--secondary', onClick: () => showToast('PDF generado') }, s4(I4, { name: 'Printer', size: 16 }), 'PDF'),
      s4('button', { key: 3, className: 'btn btn--primary', onClick: () => showToast('Reporte mensual preparado') }, s4(I4, { name: 'Sparkles', size: 16 }), 'Preparar reporte mensual'),
    ] }),
...
function Config({ showToast }) {
  ...
  return s4('main', { className: 'page fade-in' },
    s4(PH4, { route: 'configuracion', actions: [s4('span', { key: 0, className: 'badge badge--teal', style: { alignSelf: 'center' } }, s4(I4, { name: 'Lock', size: 13 }), 'Instancia privada · V1')] }),
    s4('div', { className: 'grid-2' },
      sectionCard('Datos de la comunidad', 'Building2',
        s4('div', { className: 'kv-list' },
          s4('div', { className: 'kv-row' }, s4('span', { className: 'kv-row__k' }, 'Nombre'), s4('span', { className: 'kv-row__v' }, 'Administración Condo')),
      ...
      sectionCard('Cuotas y recargos', 'Coins',
        s4('div', { style: { display: 'flex', flexDirection: 'column', gap: 4 } },
          ...
          s4('div', { className: 'row row--between', style: { padding: '12px 0 2px' } }, s4('div', null, s4('div', { className: 'fw-600', style: { fontSize: 13 } }, 'Recargo automático por mora'), s4('div', { className: 'muted', style: { fontSize: 11.5 } }, '$600 tras 15 días')), s4('button', { type: 'button', role: 'switch', 'aria-checked': sw.recargos, 'aria-label': 'Activar recargo automático por mora', className: 'switch' + (sw.recargos ? ' switch--on' : ''), onClick: () => toggle('recargos') })),
        ),
      ),
      sectionCard('Usuarios y roles', 'Users',
        s4('div', { style: { display: 'flex', flexDirection: 'column', gap: 4 } },
          [['Admin Condo', 'Administrador', 'teal'], ['Comite Condo', 'Comité', 'sky'], ['Vigilancia Condo', 'Vigilancia', 'slate'], ['Residente 02', 'Residente', 'amber']].map((u, i) =>
          ...
          s4('button', { className: 'btn btn--soft btn--sm', style: { marginTop: 10 }, onClick: () => showToast('Invitación preparada') }, s4(I4, { name: 'Plus', size: 14 }), 'Invitar usuario'),
        ),
      ),
      ...
      sectionCard('Automatizaciones', 'Bolt',
        s4('div', { style: { display: 'flex', flexDirection: 'column', gap: 4 } },
          [['recordatorios', 'Recordatorios de pago', 'Aviso 3 días antes del corte'], ['pronto', 'Descuento por pronto pago', '5% antes del día 5'], ['qr', 'Caducidad automática de accesos', 'Expira al terminar la vigencia']].map((r, i) =>
            s4('div', { key: i, className: 'row row--between', style: { padding: '11px 0', borderBottom: i < 2 ? '1px solid var(--border)' : 'none' } },
              ...
              s4('button', { type: 'button', role: 'switch', 'aria-checked': sw[r[0]], 'aria-label': r[1], className: 'switch' + (sw[r[0]] ? ' switch--on' : ''), onClick: () => toggle(r[0]) }),
            )),
        ),
      ),
      sectionCard('Respaldos y datos', 'Database',
        s4('div', { style: { display: 'flex', flexDirection: 'column', gap: 12 } },
          s4('div', { className: 'banner banner--teal', style: { fontSize: 12.5 } }, s4('span', { className: 'banner__icon' }, s4(I4, { name: 'ShieldCheck', size: 15 })), s4('div', null, s4('span', { className: 'fw-700' }, 'Instancia privada. '), 'Configuración centralizada para la operación de la comunidad.')),
          s4('div', { className: 'row', style: { gap: 8 } },
            s4('button', { className: 'btn btn--secondary btn--block', onClick: () => showToast('Datos exportados') }, s4(I4, { name: 'Download', size: 15 }), 'Exportar datos'),
            s4('button', { className: 'btn btn--ghost btn--block', onClick: () => showToast('Respaldo creado') }, s4(I4, { name: 'Database', size: 15 }), 'Respaldo'),
          ),
        ),
      ),
    ),
...
export { Comunicados as CommunicationsScreen, Accesos as AccessScreen, Reportes as ReportsScreen, Config as SettingsScreen };
