...
import * as D2 from "../data/mockData.js";
import { PageHeader as PH2, Card as C2, Badge as B2, Ic as I2, Money as M2, Avatar as AV2 } from "../components/ui.jsx";

const { createElement: se2 } = React;
// ============ COBRANZA ============
function Cobranza({ tenant, showToast }) {
  ...
  const summary = (label, val, icon, tone, sub) => se2('div', { className: 'kpi' + (tone ? ` kpi--${tone}` : '') },
    se2('div', { className: 'kpi__head' }, se2('span', { className: 'kpi__icon' }, se2(I2, { name: icon, size: 19 })), se2('span', { className: 'kpi__label' }, label), se2('button', { className: 'kebab kpi__menu', 'aria-label': `Más opciones de ${label}` }, se2(I2, { name: 'MoreVertical', size: 16 }))),
  ...
  return se2('main', { className: 'page fade-in' },
    se2(PH2, { route: 'cobranza', actions: [
      se2('button', { key: 1, className: 'btn btn--ghost', onClick: () => showToast('Excel generado') }, se2(I2, { name: 'Sheet', size: 16 }), 'Exportar Excel'),
      se2('button', { key: 2, className: 'btn btn--secondary', onClick: () => showToast('Recordatorios enviados') }, se2(I2, { name: 'Send', size: 16 }), 'Enviar recordatorios'),
      se2('button', { key: 3, className: 'btn btn--primary', onClick: () => showToast('Cuotas de junio generadas') }, se2(I2, { name: 'Coins', size: 16 }), 'Generar cuotas'),
    ] }),
      ...
      se2('div', { className: 'stack' },
        se2(C2, { title: 'Acciones masivas', icon: 'Bolt' },
          se2('div', { style: { display: 'flex', flexDirection: 'column', gap: 9 } },
            [
              ['Coins', 'Generar cuotas de junio', 'primary', 'Cuotas de junio generadas'],
              ['Send', 'Enviar recordatorios (14)', 'soft', 'Recordatorios enviados'],
              ['Sheet', 'Exportar a Excel', 'secondary', 'Excel generado'],
              ['Printer', 'Generar reporte PDF', 'secondary', 'Reporte PDF generado'],
            ].map((a, i) =>
              se2('button', { key: i, className: `btn btn--${a[2]} btn--block`, style: { justifyContent: 'flex-start' }, onClick: () => showToast(a[3]) }, se2(I2, { name: a[0], size: 16 }), a[1])),
          ),
        ),
...
function Validar({ tenant, showToast }) {
  ...
  const act = (id, label) => { setDone(d => ({ ...d, [id]: label })); setSel(null); showToast('' + label); };
  ...
  const drawer = sel && se2(React.Fragment, null,
    ...
    se2('div', { className: 'drawer' },
      se2('div', { className: 'drawer__header' },
        ...
        se2('button', { className: 'icon-btn', 'aria-label': 'Cerrar panel de pago', onClick: () => setSel(null) }, se2(I2, { name: 'X', size: 18 })),
      ),
        ...
        se2('tbody', null, rows.map((p, i) => {
          ...
          return se2('tr', { key: i, className: d ? '' : 'clickable', onClick: () => !d && setSel(p), style: d ? { opacity: 0.55 } : {} },
            ...
            se2('td', null, d ? se2('span', { className: 'subtle', style: { fontSize: 12 } }, 'Resuelto') : se2('div', { className: 'row-actions' },
              se2('button', { className: 'icon-btn', title: 'Aprobar', 'aria-label': `Aprobar pago ${p.id}`, onClick: (ev) => { ev.stopPropagation(); act(p.id, 'Pago aprobado'); }, style: { color: 'var(--green-700)' } }, se2(I2, { name: 'Check', size: 16 })),
              se2('button', { className: 'icon-btn', title: 'Rechazar', 'aria-label': `Rechazar pago ${p.id}`, onClick: (ev) => { ev.stopPropagation(); act(p.id, 'Comprobante rechazado'); }, style: { color: 'var(--red-600)' } }, se2(I2, { name: 'X', size: 16 })),
              se2('button', { className: 'icon-btn', title: 'Ver detalle', 'aria-label': `Ver detalle de pago ${p.id}`, onClick: (ev) => { ev.stopPropagation(); setSel(p); } }, se2(I2, { name: 'Eye', size: 16 })),
            )),
          );
        })),
      )),
...
export { Cobranza as CollectionsScreen, Validar as PaymentsReviewScreen };
