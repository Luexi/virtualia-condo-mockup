...
import * as DR from "../data/mockData.js";
import { Ic as IR, Logo as LogoR, Badge as BR } from "../components/ui.jsx";

const { createElement: sr } = React;
function Residente({ onBack, showToast }) {
  ...
  const inicio = sr(React.Fragment, null,
    sr('div', { className: 'r-balance' },
      sr('div', { style: { position: 'relative', zIndex: 1 } },
        ...
        sr('button', { className: 'btn btn--primary btn--block', style: { marginTop: 16 }, onClick: () => showToast('Carga de comprobante abierta') }, sr(IR, { name: 'Wallet', size: 16 }), 'Pagar ahora'),
      ),
    ),
    sr('div', { className: 'r-quick' },
      sr(Quick, { icon: 'Wallet', label: 'Pagar', onClick: () => showToast('Carga de comprobante abierta') }),
      sr(Quick, { icon: 'Wrench', label: 'Reportar', onClick: () => showToast('Incidencia registrada') }),
      sr(Quick, { icon: 'CalendarCheck', label: 'Reservar', onClick: () => showToast('Reserva preparada') }),
      sr(Quick, { icon: 'KeyRound', label: 'Visita', onClick: () => showToast('Acceso visitante preparado') }),
    ),
  ...
  const pagar = sr(React.Fragment, null,
    sr('div', { className: 'r-card' },
      ...
      sr('button', { className: 'btn btn--primary btn--block', style: { marginTop: 14 }, onClick: () => showToast('Comprobante enviado') }, sr(IR, { name: 'Send', size: 16 }), 'Enviar comprobante'),
    ...
    sr('div', { className: 'r-card' },
      ...
      sr('div', { style: { display: 'flex', flexDirection: 'column', gap: 2 } },
        [['Wrench', 'Reportar incidencia'], ['CalendarCheck', 'Reservar amenidad'], ['KeyRound', 'Autorizar visita'], ['FolderOpen', 'Documentos'], ['UserCircle', 'Mi perfil']].map((r, i) =>
          sr('button', { key: i, className: 'row row--between', style: { padding: '13px 0', borderBottom: i < 4 ? '1px solid var(--border)' : 'none', width: '100%', textAlign: 'left' }, onClick: () => showToast(r[1] + ' abierto') },
  ...
  return sr('div', { className: 'phone-stage' },
    sr('div', { style: { display: 'flex', flexDirection: 'column', gap: 16, alignItems: 'center' } },
      sr('div', { className: 'row row--between', style: { width: 390, maxWidth: '100%' } },
        ...
        sr('span', { className: 'status-tag' }, 'Portal de residente · móvil'),
      ),
      sr('div', { className: 'phone' },
        ...
        sr('div', { style: { padding: '4px 16px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
          ...
          sr('button', { className: 'icon-btn', type: 'button', 'aria-label': 'Notificaciones', style: { width: 34, height: 34 } }, sr(IR, { name: 'Bell', size: 18 })),
        ),
...
export { Residente as ResidentPortalScreen };
