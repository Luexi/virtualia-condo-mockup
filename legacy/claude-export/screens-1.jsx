/* global React, UI */
const { createElement: se1, useState: useState1 } = React;
const U1 = UI, D1 = window.DATA;
...
function Inicio({ setRoute, setDetail, showToast }) {
  const k = D1.KPIS;
  const kebab = se1('button', { className: 'kebab kpi__menu', onClick: (ev) => ev.stopPropagation() }, se1(I1, { name: 'MoreVertical', size: 16 }));
  ...
  return se1('main', { className: 'page fade-in' },
    ...
    se1('div', { className: 'page-header__row', style: { alignItems: 'flex-start' } },
      se1('div', null,
        se1('h1', { className: 'welcome-h1' }, 'Bienvenido, Admin Demo'),
      ...
      se1('div', { className: 'page-header__actions' },
        ...
        se1('button', { className: 'btn btn--dark', onClick: () => showToast('Demo: exportando resumen') }, se1(I1, { name: 'Download', size: 16 }), 'Exportar'),
      ),
    ),
    ...
    se1('div', { className: 'dash-grid' },
      ...
      se1('div', { className: 'stack' },
        ...
        se1('div', { className: 'hero' },
          ...
          se1('div', { className: 'row', style: { gap: 10, marginTop: 18 } },
            ...
            se1('button', { className: 'btn btn--secondary', style: { flex: 1 }, onClick: () => showToast('Demo: recordatorios enviados') }, se1(I1, { name: 'Send', size: 16 }), 'Recordatorios'),
          ),
          ...
          se1('div', { className: 'mini-grid' },
            ...
            ].map((m, i) => se1('div', { key: i, className: 'mini', onClick: () => setRoute(m.go) },
        ...
        se1('div', { className: 'copilot' },
          se1('div', { className: 'copilot__head' },
            ...
            se1('span', { className: 'right demo-tag', style: { background: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.8)', borderColor: 'rgba(255,255,255,0.2)' } }, 'Demo'),
          ),
          se1('div', { className: 'copilot__prompts' },
            ['Resume morosos del mes', 'Prepara reporte para comité', 'Explica variación de gastos', 'Redacta aviso de mantenimiento'].map((p, i) =>
              se1('button', { key: i, className: 'copilot__prompt', onClick: () => showToast('Demo: el copiloto generaría un borrador (solo lectura)') },
                se1(I1, { name: 'Sparkles', size: 15 }), p, se1('span', { className: 'right' }, se1(I1, { name: 'ArrowRight', size: 14 })),
              )),
          ),
          se1('div', { className: 'copilot__foot' }, se1(I1, { name: 'Info', size: 13 }), 'Modo lectura · no ejecuta acciones reales'),
        ),
      ),
      ...
      se1('div', { className: 'stack' },
        // KPI row
        se1('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18 } },
      ...
      se1(C1, { title: 'Actividad reciente', icon: 'Clock' },
        se1('div', { className: 'act-list' },
          D1.ACTIVIDAD.slice(0, 4).map((a, i) => se1('div', { key: i, className: 'act-item' },
            ...
            se1('div', { className: 'act-item__title', dangerouslySetInnerHTML: { __html: a.txt } }),
...
function Unidades({ tenant, setRoute, setDetail, showToast }) {
  ...
  return se1('main', { className: 'page fade-in' },
    se1(PH, { route: 'unidades', actions: [
      ...
      se1('button', { key: 2, className: 'btn btn--primary', onClick: () => showToast('Demo: alta de unidad') }, se1(I1, { name: 'Plus', size: 16 }), 'Nueva unidad'),
    ] }),
    se1('div', { className: 'card' },
      se1('div', { className: 'card__header', style: { flexWrap: 'wrap', gap: 12 } },
        ...
        se1('div', { className: 'search', style: { maxWidth: 240, margin: 0 } },
          ...
          se1('input', { placeholder: 'Buscar unidad…', value: q, onChange: ev => setQ(ev.target.value), style: { padding: '8px 12px 8px 38px' } }),
        ),
      ),
      se1('div', { className: 'table-wrap' },
        se1('table', { className: 'tbl' },
          ...
          se1('tbody', null, rows.map((u, i) => se1('tr', { key: i, className: 'clickable', onClick: () => { setDetail(u.u); setRoute('estado'); } },
            ...
            se1('td', null, se1('span', { className: 'mono muted' }, u.tel)),
            ...
            se1('td', null, se1('div', { className: 'row-actions' },
              se1('button', { className: 'icon-btn', title: 'Estado de cuenta', onClick: (ev) => { ev.stopPropagation(); setDetail(u.u); setRoute('estado'); } }, se1(I1, { name: 'Eye', size: 16 })),
              se1('button', { className: 'icon-btn', title: 'Registrar pago', onClick: (ev) => { ev.stopPropagation(); showToast('Demo: registrar pago de ' + u.u); } }, se1(I1, { name: 'Wallet', size: 16 })),
              se1('button', { className: 'icon-btn', title: 'Más', onClick: (ev) => ev.stopPropagation() }, se1(I1, { name: 'MoreVertical', size: 16 })),
            )),
          ))),
        ),
      ),
...
function EstadoCuenta({ detail, setRoute, showToast }) {
  ...
  return se1('main', { className: 'page fade-in' },
    se1(PH, { route: 'estado',
      ...
      actions: [
        se1('button', { key: 1, className: 'btn btn--ghost', onClick: () => showToast('Demo: enviando aviso a ' + u.u) }, se1(I1, { name: 'Send', size: 16 }), 'Enviar aviso'),
        se1('button', { key: 2, className: 'btn btn--secondary', onClick: () => showToast('Demo: PDF generado') }, se1(I1, { name: 'Printer', size: 16 }), 'Generar PDF'),
        se1('button', { key: 3, className: 'btn btn--primary', onClick: () => showToast('Demo: registrar pago') }, se1(I1, { name: 'Plus', size: 16 }), 'Registrar pago'),
      ] }),
    ...
    se1('div', { className: 'grid-main-l' },
      ...
      se1('div', { className: 'stack' },
        se1('div', { className: 'card', style: { overflow: 'hidden' } },
          ...
          se1('div', { className: 'card__body' },
            se1('div', { className: 'kv-list' },
              ...
              se1('div', { className: 'kv-row' }, se1('span', { className: 'kv-row__k' }, 'Contacto'), se1('span', { className: 'kv-row__v mono' }, u.tel)),
        ...
        se1(C1, { title: 'Notas internas', icon: 'FileText' },
          ...
          se1('button', { className: 'btn btn--soft btn--sm', style: { marginTop: 10 }, onClick: () => showToast('Nota guardada (demo)') }, 'Guardar nota'),
        ),
      ),
      ...
      se1('div', { className: 'stack' },
        ...
        se1(C1, { title: 'Comprobantes y recibos', icon: 'FolderOpen',
          headerRight: se1('button', { className: 'card__link', onClick: () => showToast('Demo: subir comprobante') }, se1(I1, { name: 'Plus', size: 14 }), 'Subir') },
...
window.Screens = Object.assign(window.Screens || {}, { Inicio, Unidades, EstadoCuenta });
