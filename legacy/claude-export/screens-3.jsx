/* global React, UI */
const { createElement: se3, useState: useState3 } = React;
const D3 = window.DATA;
const { PageHeader: PH3, Card: C3, Badge: B3, Ic: I3, Avatar: AV3 } = UI;
...
function Mantenimiento({ tenant, showToast }) {
  ...
  const ticketCard = (t) => se3('div', { key: t.id, className: 'ticket', onClick: () => showToast('Demo: abrir ' + t.id) },
  ...
  return se3('main', { className: 'page fade-in' },
    se3(PH3, { route: 'mantenimiento', actions: [
      ...
      se3('button', { key: 1, className: 'btn btn--primary', onClick: () => showToast('Demo: nuevo ticket') }, se3(I3, { name: 'Plus', size: 16 }), 'Nuevo ticket'),
    ] }),
    ...
    se3('div', { className: 'card', style: { background: 'linear-gradient(120deg, var(--teal-50), var(--surface) 60%)', borderColor: 'var(--teal-100)' } },
      se3('div', { className: 'card__body', style: { display: 'grid', gridTemplateColumns: '1fr', gap: 14 } },
        ...
        se3('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 } },
    ...
    vista === 'tablero'
      ...
      : se3('div', { className: 'card', body: false },
          se3('div', { className: 'table-wrap' }, se3('table', { className: 'tbl' },
            ...
            se3('tbody', null, tickets.map((t, i) => se3('tr', { key: i, className: 'clickable', onClick: () => showToast('Demo: abrir ' + t.id) },
...
function Albercas({ showToast }) {
  ...
  return se3('main', { className: 'page fade-in' },
    se3(PH3, { route: 'albercas', actions: [
      se3('span', { key: 0, className: 'demo-tag', style: { alignSelf: 'center' } }, 'Módulo diferenciador'),
      se3('button', { key: 1, className: 'btn btn--secondary', onClick: () => showToast('Demo: exportar bitácora') }, se3(I3, { name: 'Download', size: 16 }), 'Bitácora PDF'),
      se3('button', { key: 2, className: 'btn btn--primary', onClick: () => showToast('Demo: registrar lectura química') }, se3(I3, { name: 'Plus', size: 16 }), 'Registrar lectura'),
    ] }),
    ...
    se3('div', { className: 'grid-main' },
      ...
      se3('div', { className: 'stack' },
        ...
        se3(C3, { title: 'Evidencia fotográfica', icon: 'Image',
          headerRight: se3('button', { className: 'card__link', onClick: () => showToast('Demo: subir foto') }, se3(I3, { name: 'Plus', size: 14 }), 'Subir') },
...
function Amenidades({ showToast }) {
  ...
  return se3('main', { className: 'page fade-in' },
    se3(PH3, { route: 'amenidades', actions: [
      ...
      se3('button', { key: 1, className: 'btn btn--primary', onClick: () => showToast('Demo: nueva reserva') }, se3(I3, { name: 'Plus', size: 16 }), 'Nueva reserva'),
    ] }),
    ...
    vista === 'residente'
      ? se3('div', { className: 'card', style: { maxWidth: 560 } }, se3('div', { className: 'card__body', style: { display: 'flex', flexDirection: 'column', gap: 14 } },
          ...
          se3('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 } }, D3.AMENITIES.slice(0, 4).map((a, i) =>
            se3('button', { key: i, className: 'card', style: { padding: 14, display: 'flex', gap: 11, alignItems: 'center', textAlign: 'left' }, onClick: () => showToast('Demo: reservar ' + a.nombre) },
            ...
            se3(C3, { title: 'Reservas por aprobar', icon: 'CalendarCheck', body: false,
              ...
              se3('div', { style: { padding: '6px 8px' } }, D3.RESERVAS.map((r, i) => se3('div', { key: i, style: { padding: '12px', borderBottom: i < 3 ? '1px solid var(--border)' : 'none' } },
                ...
                r.estado === 'pendiente' && se3('div', { className: 'row', style: { gap: 6, marginTop: 8 } },
                  se3('button', { className: 'btn btn--soft btn--sm', onClick: () => showToast('Reserva aprobada (demo)') }, se3(I3, { name: 'Check', size: 14 }), 'Aprobar'),
                  se3('button', { className: 'btn btn--ghost btn--sm', onClick: () => showToast('Reserva rechazada (demo)') }, 'Rechazar'),
                ),
              ))),
            ),
          ),
...
window.Screens = Object.assign(window.Screens || {}, { Mantenimiento, Albercas, Amenidades });
